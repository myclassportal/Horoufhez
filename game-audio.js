const GameAudio = {
    ...
    numToFiles(numStr) {
        ...
        const files = [];
        const thousandsPart = s.length > 3 ? parseInt(s.slice(0, -3)) : 0;
    ...
    speakIntroDrag(onComplete) {
        this.playList(['intro_drag'], onComplete);
    },

    speakIntroType(onComplete) {
        this.playList(['intro_type'], onComplete);
    },

    speakSeparate(onComplete) {
        this.playList(['joda_kon'], onComplete);
    },

    speakThousands(thousandsChunk, onComplete) {
        const list = [
            'tabaghe_hezar',
            ...this.chunkToFiles(thousandsChunk),
            'hezar'
        ];
        this.playList(list, onComplete);
    },

    speakZeroNote(onComplete) {
        this.playList(['sefr_khande_namishe'], onComplete);
    },

    speakUnits(unitsChunk, onComplete) {
        const list = [
            'tabaghe_yeki',
            ...this.chunkToFiles(unitsChunk)
        ];
        this.playList(list, onComplete);
    },
